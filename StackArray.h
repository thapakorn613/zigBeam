...
class StackArray {
  public:
    ...
    // get an item from exact input pointer
    T getStackByPtr (const int pointer);
...
// get the pointer of the stack.
template<typename T>
int StackArray<T>::getStkPtr () {
  return top;
}

// get an item from exact input pointer
template<typename T>
T StackArray<T>::getStackByPtr (const int pointer) {
  // check if the stack is empty.
  if (isEmpty ())
